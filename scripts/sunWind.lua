--[[      Virtual Lux sensor and other real-time solar data
    ...
    -- Variables to customize ------------------------------------------------
       local localhost = 'localhost:8084'  -- Set your port. (Not the universal IP).
       local city = "Zwijndrecht"                -- Your city for Wunderground API
       local countryCode = "NL"            -- Your country code for Wunderground API
       local idxLux ='1'                  -- Your virtual Lux Device ID
       local idxSolarAzimuth ='2'         -- Your virtual Azimuth Device ID
       local idxSolarAltitude ='3'        -- Your virtual Solar Altitude Device ID
       local idxUserVarOcta='1'            -- Your user variable ID , named octa
       local idxWindSpeed= '11'
       local wuAPIkey = "6e4115998bfd203b" -- Your Weather Underground API Key
       local latitude = 51.823353          -- your home
       local longitude = 4.638844          -- your home
       local altitude = -0.9                 -- Your home altitude : run once in debug = 1 to found your altitude in Log and write it here
       local WMOID = '06356'               -- Your nearest SYNOP Station for ogimet. Very important !
       local DEBUG = 1             -- 0 , 1 for domoticz log , 2 for file log
       local weatherStation = "IHENDRIK24"
    ...
    if  ((time.min % 5)==0)  then -- Run every 5 minutes. Check the wundergroud API limitation before changing this
       ...
       --  API Wunderground
       local config=assert(io.popen('curl http://api.wunderground.com/api/'..wuAPIkey..'/conditions/q/pws:'..weatherStation..'.json'))
       ...
       local windSpeed = jsonLocation.current_observation.wind_kph / 3.6 * 10.0
       local maxWindSpeed = jsonLocation.current_observation.wind_gust_kph / 3.6 * 10.0
       local windBearing = jsonLocation.current_observation.wind_degrees
       local windDirection = jsonLocation.current_observation.wind_dir
...
--      commandArray[#commandArray + 1] = {['UpdateDevice'] = idxSolarAltitude..'|0|'..tostring(round(sunAltitude,0))}
	...
	local windSetting = tostring(windBearing)..";"..tostring(windDirection)..";"..tostring(windSpeed)..";"..tostring(maxWindSpeed)..";0;0"
	print(windSetting)
	
	commandArray[#commandArray + 1]={['OpenURL']="http://"..localhost.."/json.htm?type=command&param=udevice&idx="..idxWindSpeed.."&nvalue=0&svalue="..windSetting }
